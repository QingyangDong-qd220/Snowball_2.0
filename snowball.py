...
class Snowball(BaseSentenceParser):
    ...
    def import_data(self, path, confidence_limit=1.0):
        ...
        try:
            ...
            raise ModuleNotFoundError("Invalid checkpoint due to missing module. Try importing files created by running the export_to_list method. ")

        # check the type of other_model: list or Snowball instance; jump to import_from_list if needed
        if isinstance(other_model, list):
            # print("loading from list")
            self.import_from_list(path, confidence_limit)
            return
    ...
    def export_to_list(self, path=None):

        """
        write all phrases (including the ones assigned into clusters) to a file of a list,
        ...
        if path == None:
            path = self.model.__name__ + str("_list.pkl")
